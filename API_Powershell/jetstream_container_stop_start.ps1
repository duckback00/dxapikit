...
#
# Program Name : jetstream_container_start_stop.ps1
...
#
# Interactive Usage: 
# ./jetstream_container_start_stop.ps1
#
# Non-interactive Usage:
# ./jetstream_container_start_stop.ps1 [template_name] [container_name] [stop|start]
# ./jetstream_container_start_stop.ps1 tpl cdc stop
#
#########################################################
#                   DELPHIX CORP                        #
...
if ( "${ACTION}" -eq "stop" ) {
   $results = (curl.exe -sX POST -k ${BaseURL}/jetstream/container/${JS_CONTAINER_REF}/disable -b "${COOKIE}" -H "${CONTENT_TYPE}" -d "${json}")
}

if ( "${ACTION}" -eq "start" ) {
   $results = (curl.exe -sX POST -k ${BaseURL}/jetstream/container/${JS_CONTAINER_REF}/enable -b "${COOKIE}" -H "${CONTENT_TYPE}" -d "${json}")
}

$status = ParseStatus "${results}" "${ignore}"
Write-Output "${ACTION} Container Job Results: ${results}"
