...
#
# Interactive Usage:
# ./jetstream_create_branch_from_latest_jq.sh
#
# ... or ...
#
# Non-Interactive Usage:
# ./jetstream_create_branch_from_latest_jq.sh [template_name] [container_name] [branch_name]
#
# ./jetstream_create_branch_from_latest_jq.sh tpl cdc default
# ./jetstream_create_branch_from_latest_jq.sh tpl cdc 
#
#########################################################
#                   DELPHIX CORP                        #
...
#echo "Getting Jetstream Branches ..."
...
#
JS_BRANCH_REF=`echo "${STATUS}" | jq --raw-output '.result[] | select (.name=="'"${BRANCH_NAME}"'") | .reference '`
if [[ "${JS_BRANCH_REF}" != "" ]] 
then
   echo "Error: Branch Name Already Exists, ${BRANCH_NAME} with reference ${JS_BRANCH_REF}, exiting"
   exit 1;
fi
