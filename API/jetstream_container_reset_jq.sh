...
#
# Program Name : jetstream_container_reset_jq.sh 
...
#
# Interactive Usage: 
# ./jetstream_container_reset_jq.sh
#
# Non-interactive Usage:
# ./jetstream_container_reset_jq.sh [template_name] [container_name]
# ./jetstream_container_reset_jq.sh tpl cdc
#
#########################################################
#                   DELPHIX CORP                        #
...
STATUS=`curl -s -X POST -k --data @- $BaseURL/jetstream/container/${JS_CONTAINER_REF}/reset -b "${COOKIE}" -H "${CONTENT_TYPE}" <<EOF
${json}
EOF
`

#echo "JetStream Container Reset Results: ${STATUS}"
RESULTS=$( jqParse "${STATUS}" "status" )
