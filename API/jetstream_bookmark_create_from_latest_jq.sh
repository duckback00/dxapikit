...
#
# Program Name : jetstream_bookmark_create_from_latest_jq.sh 
...
#
# Interactive Usage: 
# ./jetstream_bookmark_create_jq.sh
#
# Non-interactive Usage:
# ./jetstream_bookmark_create_from_latest_jq.sh [template_name] [container_name] [bookmark_name]    SHARED         TAGS
# ./jetstream_bookmark_create_from_latest_jq.sh [template_name] [container_name] [bookmark_name] [true|false] ["tag1","tag2"] 
#
# Tags are arrays and must be "quoted" if more than one and delimited by a comma
# 
# ./jetstream_bookmark_create_from_latest_jq.sh tpl dc BM3 false '"Hey","There"'
#
# Non-interactive using hardcode defaults iff set: 
# ./jetstream_bookmark_create_from_latest_jq.sh 
#
#########################################################
#                   DELPHIX CORP                        #
...
then
   ...
   then  
      echo "---------------------------------"
      echo "Each value must be quoted and multiple tags delimited by comma"
      echo "   Single Tag Example: \"REL1.3\""
      echo "Multiple Tags Example: \"API Created\",\"REL123\""
      echo "Please Enter Bookmark Tags (Enter for no tags): "
      read TAGS
      #if [[ "${TAGS}" == "" ]]
      #then
