...
DRIP="172.16.160.134"		# Delphix Reporting Server IP Address/FullyQualified Hostname
...
#RPTLIST=`echo "${STATUS}" | ${JQ} -r '.[] | select (._id | contains("month")) | ._id'`
RPTLIST=`echo "${STATUS}" | ${JQ} -r '.[] | select(._id | startswith("result_month_minus")) | ._id'`
